...
@section('title', 'Create Blog Post - NGO Australia')
...
<span class="font-bold text-lg">Create Blog Post</span>
</div>
<div class="hidden lg:flex items-center gap-2">
<h2 class="text-lg font-bold">Create Blog Post</h2>
<span class="bg-primary/10 text-primary px-2 py-0.5 rounded text-xs font-semibold">New Post</span>
</div>
...
<h1 class="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Create New Blog Post</h1>
<p class="text-slate-500 dark:text-text-secondary max-w-xl">Share inspiring stories, updates, and impact from the field with your community.</p>
</div>
...
<form id="blogForm" method="POST" action="{{ route('admin.blog.store') }}" enctype="multipart/form-data" class="space-y-8">
@csrf
...
<input type="text" name="title" value="{{ old('title') }}" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-3 text-white placeholder-text-secondary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" placeholder="Enter an inspiring title..." required>
</div>
<div>
<label class="block text-text-secondary text-xs font-bold uppercase tracking-wider mb-2">Slug *</label>
<input type="text" name="slug" value="{{ old('slug') }}" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-3 text-white placeholder-text-secondary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" placeholder="url-friendly-slug" required>
</div>
<div>
<label class="block text-text-secondary text-xs font-bold uppercase tracking-wider mb-2">Excerpt *</label>
<textarea name="excerpt" rows="3" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-3 text-white placeholder-text-secondary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all resize-none" placeholder="Brief summary of your post..." required>{{ old('excerpt') }}</textarea>
</div>
<div>
<label class="block text-text-secondary text-xs font-bold uppercase tracking-wider mb-2">Content *</label>
<textarea name="content" rows="12" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-3 text-white placeholder-text-secondary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all resize-none" placeholder="Write your story here..." required>{{ old('content') }}</textarea>
</div>
</div>
</div>
...
<p class="text-sm text-text-secondary">Click to upload featured image</p>
...
<input type="radio" name="status" value="draft" {{ old('status', 'draft') === 'draft' ? 'checked' : '' }} class="text-primary focus:ring-primary">
<span class="ml-2 text-text-secondary">Draft</span>
</label>
<label class="flex items-center">
<input type="radio" name="status" value="published" {{ old('status') === 'published' ? 'checked' : '' }} class="text-primary focus:ring-primary">
<span class="ml-2 text-text-secondary">Published</span>
</label>
<label class="flex items-center">
<input type="radio" name="status" value="archived" {{ old('status') === 'archived' ? 'checked' : '' }} class="text-primary focus:ring-primary">
...
<option value="volunteers" {{ old('category') === 'volunteers' ? 'selected' : '' }}>Volunteers</option>
...
<input type="text" name="author_name" value="{{ old('author_name', 'NGO Australia') }}" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-2 text-white placeholder-text-secondary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm" required>
</div>
<div>
<label class="block text-text-secondary text-xs font-bold uppercase tracking-wider mb-1">Author Role</label>
<input type="text" name="author_role" value="{{ old('author_role') }}" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-2 text-white placeholder-text-secondary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm" placeholder="e.g., Community Director">
</div>
</div>
</div>
...
<input type="datetime-local" name="published_at" value="{{ old('published_at') }}" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm">
...
<input type="number" name="read_time" value="{{ old('read_time') }}" min="1" class="w-full bg-[#1e261c] border border-[#42523d] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm" placeholder="5">
</div>
</div>
</div>
...
<button type="button" onclick="document.getElementById('blogForm').querySelector('input[name=status]').value='draft'; document.getElementById('blogForm').submit();" class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-[#42523d] text-text-secondary hover:text-white hover:border-white/20 text-sm font-bold transition-all">
<span class="material-symbols-outlined text-[18px]">save</span>
Save Draft
</button>
</div>
